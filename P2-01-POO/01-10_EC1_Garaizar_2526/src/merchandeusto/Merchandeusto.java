...
public class Merchandeusto {
	...
	public static Pedido pedidoMasCaro(ArrayList<Pedido> pedidos) {
		double totalPedidoMasCaro = pedidos.get(0).totalPedido();
		Pedido pedidoMasCaro = pedidos.get(0);
		for(Pedido p: pedidos) {
			if (p.totalPedido() > totalPedidoMasCaro) {
				totalPedidoMasCaro = p.totalPedido();
				pedidoMasCaro = p;
			}
		}
...
//		// Debería mostrar el pedido con el código 4
		System.out.println(pedidoMasCaro(pedidos).getCodigo());
//		
//		// Ejercicio 5: filtrarRopa
//		// Debería mostrar solo las camisetas y sudaderas
	...
	private static ArrayList<Producto> filtrarRopa(ArrayList<Producto> productos) {
		ArrayList<Producto> resultado = new ArrayList<Producto>();
		
		for (Producto p : productos) {
			if(p instanceof Camiseta || p instanceof Sudadera) {
				resultado.add(p);
			}
		}
		return resultado;
