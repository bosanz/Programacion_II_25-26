...
public class Partida {
    ...
    private void turno() {
        ...
        intentosTotales++;
        if (!estabaResuelto && puzzle.isResuelto()) puzzlesResueltos++;

        System.out.println("Sala: " + salaActual.getNombre()
                + " | Jugador: " + jugador.getNombre()
                + " | Puzzle: " + puzzle.getId()
                + (puzzle.isObligatorio() ? " (OBL)" : " (OPT)")
                + " | Intento #" + puzzle.getIntentos()
                + " | " + (exito ? "ÉXITO" : "FALLO")
                + " | Tiempo restante: " + tiempoRestanteMin + " min");
    }
    ...
    public boolean haTerminado() {
        if (escapeRoom == null) return true;
        if (salaActual == null) return true;      // victoria
