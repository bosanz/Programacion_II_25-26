...
public class EscapeRoomApp {
    ...
    private static EscapeRoom construirEscapeRoomDemo() {
        ...
        s1.addPuzzle(new Puzzle("P1", "Encuentra el código del candado.", true, "CORRECTA"));
        s1.addPuzzle(new Puzzle("P2", "Alinea símbolos para activar el mecanismo.", false, 40));
        room.addSala(s1);

        Sala s2 = new Sala("Galería", 2);
        s2.addPuzzle(new Puzzle("P3", "Resuelve el panel numérico.", true, "CORRECTA"));
        s2.addPuzzle(new Puzzle("P4", "Reconstruye el mosaico.", true, 45));
        s2.addPuzzle(new Puzzle("P5", "Abre el compartimento oculto.", false, 35));
        room.addSala(s2);

        Sala s3 = new Sala("Cámara Final", 2);
        s3.addPuzzle(new Puzzle("P6", "Activa los interruptores en orden.", true, 50));
        s3.addPuzzle(new Puzzle("P7", "Introduce la clave de salida.", true, "CORRECTA"));
        room.addSala(s3);
